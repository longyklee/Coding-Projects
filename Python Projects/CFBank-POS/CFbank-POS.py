...
input("Press Enter to enter the CFbank (FUSION) POS")
...
if specialitemyn == 'y':
    ...
    class Register:
        def __init__(self, parent):
            self.parent = parent
            parent.title('CFbank (FUSION) POS ('+cashier+' LOGGED IN)')
            self.font = ('Courier New', 12)
            self.till = 0
            self.TAX = 0.095
            self.items = {'fusionpro':Item('Fusion Pro', 129900,
                                          tk.Button(root,
                                          text='Fusion Pro',
                                          command=lambda: self.scan('fusionpro'),
                                          font=self.font)),
                          'fusionlite':Item('Fusion Lite', 99900,
                                          tk.Button(root,
                                          text='New Item',
                                          command=lambda: self.scan('fusionlite'),
                                          font=self.font)),
                           'special':Item(specialname, specialcost,
                                          ...
                                          font=self.font)),
                           'fusioncom':Item('Fusion COM', 20000,
                                         tk.Button(root,
                                         text='Fusion COM',
                                         command=lambda: self.scan('fusioncom'),
                                         font=self.font)),
                           'max':Item('   Max', 20000,
                                         tk.Button(root,
                                         text='Max',
                                         command=lambda: self.scan('max'),
                                         font=self.font)),
                            'camera':Item('   Camera', 10000,
                                         tk.Button(root,
                                         text='Camera',
                                         command=lambda: self.scan('camera'),
                                         font=self.font)),
                            'spacegray':Item('   Space Gray', 100,
                                         tk.Button(root,
                                         text='Space Gray',
                                         command=lambda: self.scan('spacegray'),
                                         font=self.font)),
                          'sliver':Item('   Sliver', 150,
                                         tk.Button(root,
                                         text='Sliver',
                                         command=lambda: self.scan('sliver'),
                                         font=self.font)),
                          'rosegold':Item('   Rose Gold', 0,
                                         tk.Button(root,
                                         text='Rose Gold',
                                         command=lambda: self.scan('rosegold'),
                                         font=self.font))}
